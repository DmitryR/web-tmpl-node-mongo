...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 3. if evryhing is ok, send token to client
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('Your are not logeed in! Please login to get access.', 401)
    );
  }
...
exports.restrictTo = (...roles) => {
  return catchAsync(async (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You do no have a permission to perform this action', 403)
      );
    }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message =
    `Forgot your password? Click <a href="${resetURL}">Reset password</a> and enter new password.<br />` +
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  console.log(user);
  // 2. If token is not expires, and ther is user, set the new password
  if (!user) {
